...
状态：暂缓冻结。A2 在 Recall@10 与 Recall@20 上超过 R0，并消除了死专家；但 R1.2 的 Recall@20 仍更高，A2 的头部排序仍未超过 R0。
...
单 Adapter 短跑、A1 负载均衡和 A2 自适应分组均已完成。A1 证明固定 `moe_loss_weight=0.01` 能显著改善负载，但会损伤 R1.2 的 Recall@20；A2 将一个高共激活专家对分为同组，保持 8/8 专家活跃，并相对 A1 全面恢复推荐指标，但 Recall@20 仍未达到 R1.2。

下一步先讨论是否只追加一个低成本、单变量实验：在 A2 分组结构不变的前提下，将负载均衡权重降低或在分组冻结后退火，使专家在避免重新塌缩的同时恢复专业化。结构决定前不运行 CA、TKY，不接入 LLM，也不增加种子。未来 LLM 仍作为离线教师，对冻结后的组级语义分布或属性进行监督；专家数量变化时重新执行 warm-up 分组即可，不要求重写 LLM 接口。
