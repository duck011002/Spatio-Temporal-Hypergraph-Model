...
状态：第一版代码、服务器 smoke 与单种子 NYC 完整训练均已完成；按当前快速成功判据未通过。
...
R1 快速成功判据：在同一数据划分、同一候选集和同一单次运行口径下，Recall@1、Recall@5、Recall@10 三项中至少两项高于 R0。Recall@20 同样记录，但不计入这一条二选三判据。NDCG@1、NDCG@5、NDCG@10、NDCG@20、MAP@1/5/10/20 与 MRR 必须完整保存，用于判断排序质量是否与命中率一致；单种子结果只用于工程筛选，不声明统计显著。
...
| Recall@20 | 0.6688938141 | 0.6755753756 | +0.0066815615 | 是，不计入二选三 |
...
判定：Recall@1、Recall@5、Recall@10 中只有 Recall@10 超过 R0，即 `1/3`，因此 R1 第一版未达到快速成功条件。Recall@10 与 Recall@20 同时提高约 0.00668，说明候选覆盖范围存在正向信号；但 NDCG、MAP、MRR 和头部 Recall 下降，说明当前 MoE 残差更像扩大长列表覆盖，尚未改善前排排序。这一解释只针对当前单种子结果。
...
R0 的 NYC 基线已经完成并作为固定门槛。R1 第一版工程链路完整，但按 Recall@1/@5/@10 的二选三规则未通过，不直接扩展到 CA、TKY。下一轮优先削弱 MoE 残差对头部排序的扰动，再用相同 NYC 单种子快速筛选。LLM 教师仍未开始，相关收益为 `[NEEDS DATA]`。
